...
PDF extraction uses **MinerU** (`mineru-open-api` CLI) as the first-pass extractor for both text-based and scanned PDFs. MinerU handles OCR, formula recognition, and image extraction, but table recognition is disabled in this question-bank workflow so that table data is kept as an image instead of being reconstructed as text or an HTML/Markdown table. MinerU does not replace vault-local review: question text must still be checked against the original PDF, and every extracted or manually cropped image must be verified against the source. Supports PDF, images, Word, Excel, PPT → Markdown, HTML, LaTeX, DOCX.
...
mineru-open-api extract 文件.pdf -o ./out -f md,docx --table=false
```
...
- Table recognition disabled; crop every table from the source PDF and embed it as an image
...
1. Identify the paper metadata.
   - Use `YYYYMM-组织-年级-类型` as the paper prefix, for example `202604-湖丽衢-高三-二模`.
...
2. Extract text and images with MinerU.
   ...
   - Never paste raw MinerU output directly into a final question note. Re-layout against the original PDF: separate stem, options, subquestions, code blocks, table images, and other image references.
   ...
   - Convert each continuous program to one fenced `python` block. Preserve triple-quoted strings/comments in their original position because they often define input data or data structure, and preserve output statements at the end of the program. Keep them in the same code block as the main code.
   - Use inline code for any question-stem fragment containing Markdown-sensitive code symbols such as `_`, `*`, `[]`, or `/`, for example `a[i]`, `df["列名"]`, `x/y`, and `left_right`. Wrap the complete relevant expression rather than only the individual symbol.
   - For non-choice questions, lay out subquestions per the original paper structure (e.g. `(1)`, `(2)`, `①`, `②`). Put every fill-in marker such as `①`, `②`, or `③` between visible blank lines on both sides, for example `______①______`.
   - Keep the question number in frontmatter, filename, and the `# 题目 xx` heading, but do not repeat the source-paper question number at the beginning of the question body. Remove leading forms such as `15.`、`15、` or `第15题`.
   - Do not read, transcribe, or reconstruct table data as text, HTML, or a Markdown table. Crop the complete table from the source PDF and embed it at the corresponding position as an image.
...
4. Handle material question groups.
   - If one shared material block supports multiple choice questions, include the complete shared material in every generated note, but include only the current numbered question and its options.
   - Do not copy the other questions, options, answers, or explanations from the same material group into the current note.
   - Rewrite range wording such as `回答第 X 至 Y 题` so that the single-question note points only to its current question number.
   - Duplicate the note once per related question.
   - Change `id`, filename, title question number, current question content, primary classification, answer, and explanation for each duplicate.
   - Use `## 相关链接` to link sibling notes from the same material group when useful; links do not replace or expand the current question content.
...
5. Use the current template fields.
   ...
   - New questions start with `完成次数: 0`, blank `补课:`, blank `正确率:`, and blank `错题原因:`.
...
6. Handle images.
   ...
   - Reuse MinerU's extracted image as-is when it is clear, complete, and correctly bounded. If MinerU produces several usable separate images, do not deliberately stitch them back together.
   - When MinerU output is unusable and several related subfigures, option images, or diagrams are arranged on the same row in the original PDF, prefer one manual crop covering the whole row. Do not mechanically split that row into several unattractive fragments.
   - Preserve the complete visual unit when cropping, including its lower border, labels, arrows, axes, legends, captions, and necessary surrounding identifiers. Extend the bottom boundary when needed rather than clipping the final row or caption.
   - Always present table data as a complete local image. Do not use MinerU/OCR output to recreate or manually read the table into Markdown/HTML.
   - Do not replace required diagrams, charts, tree/stack/queue/link-list structures, UI screenshots, or unreadable code screenshots with text only.
   - Do not omit code when MinerU represents it as an image or loses its layout. Transcribe reliably readable code into a fenced `python` block; otherwise keep a clear local code screenshot.
   - Never save a full-page screenshot as a question image. Do not add captions below images.
   - **Image verification**: After embedding, open the final embedded image and compare it with the original PDF. Check completeness, correct grouping, readable text, unclipped borders/headers/axes/legends/captions, and absence of unrelated content.
   ...
   - During one continuous user-requested processing round, append every processed paper to `图片核验.md` and `题目核验.md`. Do not clear or replace earlier papers after each individual paper.
   - Clear the previous verification round only once when starting a genuinely new processing round. The completed verification pages must cover all papers processed in the current round.
...
8. Update management surfaces when fields change.
   ...
   - If importing practice statistics, write `完成次数`, `正确率`, and `错题原因` directly into frontmatter.
   - For completed choice-question imports, set `完成次数: 1` and `正确率` from the provided table; fill `错题原因` only when the source provides an error reason.
...
- Confirm every new note has `题型`, `来源`, `试卷`, `年级`, `题号`, `知识点`, `完成次数`, `补课`, `正确率`, `图片核验`, and `tags`.
...
- Confirm each material-choice note contains the complete shared material but only its own numbered question, options, answer, and explanation.
- Confirm each continuous program is in one fenced code block, with triple-quoted input/data-structure text and final output statements preserved.
- Confirm each `①`/`②`/`③` fill-in marker uses the exact two-sided blank form `______①______`; Markdown-sensitive fragments containing `_`, `*`, `[]`, or `/` use inline code.
- Confirm the question body does not begin by repeating the source-paper question number.
- Confirm every source table is embedded as a complete local image and no table data was reconstructed as Markdown/HTML text.
- Confirm image notes embed only local question images, preserve complete visual boundaries, follow the same-row cropping rule, and have correct `图片核验`.
- Confirm `图片核验.md` and `题目核验.md` cover every paper processed in the current continuous round, grouped by paper.
